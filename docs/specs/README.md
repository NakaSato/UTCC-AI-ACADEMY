...
- [SPEC-0052 — Vue islands: the mount contract, the registry, and what an island may not do](spec-vue-islands.md) — accepted; single-file components compiled at build time, one entrypoint that owns mounting, copy from the locale files, and every island an enhancement over markup that already works
- [SPEC-0053 — The notification history screen, and the bell that links to it](spec-notification-history.md) — accepted; /notifications is the reader's whole history, paged and read-only, the bell keeps its eight and gains the link past them, and the frame endpoint moves to /notifications/bell
- [SPEC-0054 — A teacher's authority over the course they teach](spec-teaching-course-authority.md) — accepted; their own course and no other, six writable fields while it is unpublished, and a transition request nobody may decide for themselves
- [SPEC-0055 — Retiring a lesson without retiring its history](spec-lesson-retirement.md) — accepted; a retired lesson leaves the syllabus and every denominator, keeps counting for whoever already finished it, and stays nameable so a completion and an integrity case can say which lesson they meant
- [SPEC-0056 — Public community project sharing, collaboration, and moderation](spec-community-project-sharing.md) — draft; owner-controlled publication and join approval with administrator-only moderation, awaiting the required product, academic, privacy, security, admin, and QA decisions
