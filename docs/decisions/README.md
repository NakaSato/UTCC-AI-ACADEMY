...
- [ADR-0049 — Give a proposal an answer before giving the public a platform](adr-0049-proposal-triage-before-public-platform.md) — accepted; the M13 gate document. Three of four proposal statuses are unreachable and the outcome's baseline is zero, so the first increment is triage, not participation
- [ADR-0050 — Give a list a page, and stop the audit log truncating](adr-0050-paged-lists.md) — accepted; one `Page` object and no gem, `?page=` beside the filters, an impossible page clamps to a real one, and `AuditEvent::RECENT` goes so the older half of the log stops being unreachable
- [ADR-0051 — Take Vue for islands, on the runtime build, behind one Stimulus bridge](adr-0051-vue-islands.md) — accepted; the runtime-only build because the CSP blocks the compiler, one Stimulus bridge that mounts and unmounts with Turbo, a named registry rather than imports in markup, and Stimulus stays the default
- [ADR-0052 — Give a notification somewhere to be read after the ninth](adr-0052-notification-history.md) — accepted; the bell caps at eight and nothing reaches the ninth, and because production email is deferred the bell is the only channel. Five questions carry recommendations and none is accepted
- [ADR-0053 — Build the island layer with Vite, and leave the rest on import maps](adr-0053-vite-island-build.md) — accepted; the compiler moves to the build so single-file components work inside the CSP, Vue leaves the import map, Node is a build dependency only, and npm audit joins the security gate
- [ADR-0054 — Give a teacher the course they teach, and keep the second pair of eyes](adr-0054-teaching-course-authority.md) — accepted; a teacher edits the numbers of the course they teach before it is published, and requests its lifecycle through the queue an administrator decides. Course creation waits on course copy leaving the locale files, and certificate approval stays refused while ADR-0011 stands
- [ADR-0055 — Retire a lesson rather than delete it](adr-0055-lesson-retirement.md) — accepted; `topic.destroy` either erases what learners finished or breaks a foreign key, so removal is a retirement: `retired_at` leaves every denominator and keeps counting for whoever already finished, decided in the queue like an addition. Un-retiring is deliberately absent
- [ADR-0056 — Define public community project sharing and collaboration boundary](adr-0056-community-project-sharing.md) — draft; owner-controlled publication and join requests, administrator-only moderation, and the outstanding Plan, privacy, academic, and operational decisions
