...
- [ADR-0055 — Retire a lesson rather than delete it](adr-0055-lesson-retirement.md) — draft; `topic.destroy` either erases what learners finished or breaks a foreign key, so removal is unbuilt. Recommends a `retired_at` that leaves the denominator but keeps counting for whoever already finished, decided in the queue like an addition
