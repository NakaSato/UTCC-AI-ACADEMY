class CreateSolidQueueTables < ActiveRecord::Migration[8.1]
  # Solid Queue used to live in a database of its own, loaded from
  # db/queue_schema.rb rather than migrated. It moved in here when production
  # became a managed Postgres with twenty connections, eleven of them already
  # taken: a separate database means a separate connection pool per process, and
  # four pools did not fit in what was left. One database is one pool.
  #
  # This is that schema file turned into a migration, unchanged apart from the
  # form. The tables are Solid Queue's, so leave them alone — the gem owns their
  # shape, and the way to change it is to upgrade the gem and re-import.
  def change
