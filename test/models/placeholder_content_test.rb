...
class PlaceholderContentTest < ActiveSupport::TestCase
  ...
  test "the second real topic has its own bilingual content and grading" do
    content = LessonContent.for("1-2")

    assert_predicate content, :real?
    assert_equal 4, content.options.size
    assert_equal 3, content.checks.size
    assert_not_equal LessonContent.for("1-1").translate("theory.heading"), content.translate("theory.heading")
    assert content.grade_quiz(content.correct_option)[:passed]
    assert content.grade_code(content.solution)[:passed]
  ...
  test "the first machine learning topic has its own bilingual content and grading" do
    content = LessonContent.for("2-1")

    assert_predicate content, :real?
    assert_equal 4, content.options.size
    assert_equal 3, content.checks.size
    assert_not_equal LessonContent.for("1-3").translate("theory.heading"), content.translate("theory.heading")
    assert content.grade_quiz(content.correct_option)[:passed]
    assert content.grade_code(content.solution)[:passed]
    assert_not content.grade_code(content.starter_code)[:passed]
  ...
  test "the second machine learning topic has its own bilingual content and grading" do
    content = LessonContent.for("2-2")

    assert_predicate content, :real?
    assert_equal 4, content.options.size
    assert_equal 3, content.checks.size
    assert_not_equal LessonContent.for("2-1").translate("theory.heading"), content.translate("theory.heading")
    assert content.grade_quiz(content.correct_option)[:passed]
    assert content.grade_code(content.solution)[:passed]
    assert_not content.grade_code(content.starter_code)[:passed]
  ...
  test "the third machine learning topic has its own bilingual content and grading" do
    content = LessonContent.for("2-3")

    assert_predicate content, :real?
    assert_equal 4, content.options.size
    assert_equal 3, content.checks.size
    assert_not_equal LessonContent.for("2-2").translate("theory.heading"), content.translate("theory.heading")
    assert content.grade_quiz(content.correct_option)[:passed]
    assert content.grade_code(content.solution)[:passed]
    assert_not content.grade_code(content.starter_code)[:passed]
  ...
  test "the fourth machine learning topic has its own bilingual content and grading" do
    content = LessonContent.for("2-4")

    assert_predicate content, :real?
    assert_equal 4, content.options.size
    assert_equal 3, content.checks.size
    assert_not_equal LessonContent.for("2-3").translate("theory.heading"), content.translate("theory.heading")
    assert content.grade_quiz(content.correct_option)[:passed]
    assert content.grade_code(content.solution)[:passed]
    assert_not content.grade_code(content.starter_code)[:passed]
  ...
  test "the first scikit-learn topic has its own bilingual content and grading" do
    content = LessonContent.for("3-1")

    assert_predicate content, :real?
    assert_equal 4, content.options.size
    assert_equal 3, content.checks.size
    assert_not_equal LessonContent.for("2-4").translate("theory.heading"), content.translate("theory.heading")
    assert content.grade_quiz(content.correct_option)[:passed]
    assert content.grade_code(content.solution)[:passed]
    assert_not content.grade_code(content.starter_code)[:passed]
  ...
  test "the second scikit-learn topic has its own bilingual content and grading" do
    content = LessonContent.for("3-2")

    assert_predicate content, :real?
    assert_equal 4, content.options.size
    assert_equal 3, content.checks.size
    assert_not_equal LessonContent.for("3-1").translate("theory.heading"), content.translate("theory.heading")
    assert content.grade_quiz(content.correct_option)[:passed]
    assert content.grade_code(content.solution)[:passed]
    assert_not content.grade_code(content.starter_code)[:passed]
  ...
  test "the first generative AI topic has its own bilingual content and grading" do
    content = LessonContent.for("4-1")

    assert_predicate content, :real?
    assert_equal 4, content.options.size
    assert_equal 3, content.checks.size
    assert_not_equal LessonContent.for("3-2").translate("theory.heading"), content.translate("theory.heading")
    assert content.grade_quiz(content.correct_option)[:passed]
    assert content.grade_code(content.solution)[:passed]
    assert_not content.grade_code(content.starter_code)[:passed]
  ...
  test "the second generative AI topic has its own bilingual content and grading" do
    content = LessonContent.for("4-2")

    assert_predicate content, :real?
    assert_equal 4, content.options.size
    assert_equal 3, content.checks.size
    assert_not_equal LessonContent.for("4-1").translate("theory.heading"), content.translate("theory.heading")
    assert content.grade_quiz(content.correct_option)[:passed]
    assert content.grade_code(content.solution)[:passed]
    assert_not content.grade_code(content.starter_code)[:passed]
  ...
  test "the first business AI topic has its own bilingual content and grading" do
    content = LessonContent.for("5-1")

    assert_predicate content, :real?
    assert_equal 4, content.options.size
    assert_equal 3, content.checks.size
    assert_not_equal LessonContent.for("4-2").translate("theory.heading"), content.translate("theory.heading")
    assert content.grade_quiz(content.correct_option)[:passed]
    assert content.grade_code(content.solution)[:passed]
    assert_not content.grade_code(content.starter_code)[:passed]
    assert_not content.grade_code("annual_net_value = annual_benefit + annual_cost")[:passed]
